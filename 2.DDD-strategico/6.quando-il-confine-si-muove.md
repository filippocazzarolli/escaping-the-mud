...
Lo strategico chiude i confini. Il [tattico](../3.DDD-tattico/README.md) dà forma alle regole *dentro* un contesto.

```text
Strategic Design
─────────────────────────────────────────────
Sottodominio (problem space)
   → analisi del business, non ha relazioni di integrazione

Bounded context (solution space)
   → collegati tra loro tramite Context Map
   → ciascuno ha la propria Ubiquitous Language
                    ↓
              (bridge verso)
                    ↓
Tactical Design
...
Entity, value object e aggregate arrivano subito. Repository ed eventi di dominio nel corso stanno più avanti — [persistenza](../7.Persistenza/README.md) ed [eventi](../8.Eventi/README.md) — dopo l'esagono.
