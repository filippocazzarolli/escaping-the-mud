...
Non è JPA embeddable, non è `record` vs `class`: come `Denaro` finisca in due colonne lo dice il [percorso sulla persistenza](../7.Persistenza/1.aggregate-non-e-tabella.md). Il linguaggio è un dettaglio. Il vincolo no: se il tipo lo accetta, il resto del modello lo eredita.
